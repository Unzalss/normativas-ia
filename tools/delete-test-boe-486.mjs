...
const TARGET_CODIGO = "TEST-BOE-486";
...
async function fetchTargetNormas(supabase) {
  const { data, error } = await supabase
    .from("normas")
    .select("id,codigo,titulo,estado_ingesta,num_fragmentos,document_hash")
...
function printPlan({ normas, fragmentCount, confirmDelete }) {
  console.log("\n[DELETE_TEST_BOE_486] Plan de borrado seguro");
  console.log(`CÃ³digo objetivo: ${TARGET_CODIGO}`);
  console.log(`Modo: ${confirmDelete ? "CONFIRM_DELETE" : "DRY_RUN"}`);
  console.log(`Normas encontradas: ${normas.length}`);

  normas.forEach((norma) => {
    console.log(`- id=${norma.id} | codigo=${norma.codigo} | estado=${norma.estado_ingesta || "N/D"} | fragmentos_registrados=${norma.num_fragmentos ?? "N/D"}`);
    console.log(`  titulo=${norma.titulo || "N/D"}`);
  });
...
async function deleteTargetNorma() {
  ...
  console.log("\n[DELETE_TEST_BOE_486] Borrado completado");
...
deleteTargetNorma().catch((error) => {
  console.error(`[DELETE_TEST_BOE_486][ERROR] ${error.message}`);
  process.exit(1);
});
